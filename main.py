...
def send_email(subject, message, to_address):
    ...
    from_address = 'brian@smartlivingsaves.com'
    password = os.getenv("EMAIL_PASS")
    msg = MIMEMultipart()
    msg['From'] = "smartlivingsaves.com - Email verification <" + from_address + ">"
    msg['To'] = to_address
    msg['Subject'] = subject
    msg.attach(MIMEText(message, 'html'))
    server = smtplib.SMTP_SSL('smtp.gmail.com', 578)
...
async def create_lead(lead: LeadSchema):
    ...
    msg = f'<p>Welcome to SmartLivingSaves.com, {lead.name}!</p><p>Please click on the following link to verify your email:</p><a href="{email_base_url}/verify_client?token={token}&email={quote(lead.email)}&phone={quote(lead.phone)}&db_type=leads">Verify Email</a><p>Thank you,</p><p>SmartLivingSaves.com Team</p>'
...
async def send_verification(email: EmailSchema):
    ...
    # [Rest of your email generation and sending logic]
    msg = f'<p>Welcome to SmartLivingSaves.com!</p><p>Please click on the following link to verify your email:</p><a href="{email_base_url}/verify_client?token={token}&email={quote(email.email)}&db_type=users">Verify Email</a><p>Thank you,</p><p>SmartLivingSaves.com Team</p>'
    subject = 'Email verification'
...
async def verify_client(token: str, email: str, phone: Optional[str] = None, db_type: str = "users"):
    ...
    if record:
        if record.get('verified', False):
            ...
            <a href="https://app.smartlivingsaves.com">Click here to login</a>
            """
        else:
